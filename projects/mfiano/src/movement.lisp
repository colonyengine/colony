      ...
      (case %input-method
        (:keyboard
         (when (input-enabled-p context '(:key :w))
           (fl.comp.transform:translate %transform (v3:scale (v3:make 0 1 0) velocity)))
         (when (input-enabled-p context '(:key :s))
           (fl.comp.transform:translate %transform (v3:scale (v3:make 0 -1 0) velocity)))
         (when (input-enabled-p context '(:key :a))
           (fl.comp.transform:translate %transform (v3:scale (v3:make -1 0 0) velocity)))
        ...
        (:analog
         (let ((x (get-gamepad-axis context :gamepad1 :left-horizontal))
           ...
           (fl.comp.transform:translate %transform (v3:scale (v3:make x y 0) velocity))))))))
