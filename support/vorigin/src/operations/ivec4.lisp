...
range of each component is bounded by MIN and MAX."
  ...
  (let ((diff (com:smod32- max min)))
    (com:cwset 4 out nil (com:smod32+ min (cl:random diff))))
  out)
...
and VEC2."
  (declare (optimize speed))
  (com:cwset 4 out (vec1 vec2) (com:smod32+ vec1 vec2))
  out)
...
and VEC2."
  (declare (optimize speed))
  (com:cwset 4 out (vec1 vec2) (if (zerop vec2) 0 (com:smod32/ vec1 vec2)))
  out)
...
vector VEC."
  (declare (optimize speed))
  (com:cwset 4 out vec (com:smod32* vec scalar))
  out)
...
(defun dot (vec1 vec2)
  ...
  (with-components ((v1 vec1) (v2 vec2))
    (com:smod32+ (com:smod32+ (com:smod32* v1x v2x)
                              (com:smod32* v1y v2y))
                 (com:smod32+ (com:smod32* v1z v2z)
                              (com:smod32* v1w v2w)))))
...
(defun length-squared (vec)
  ...
  (with-components ((v vec))
    (com:smod32+ (com:smod32+ (cl:expt vx 2)
                              (cl:expt vy 2))
                 (com:smod32+ (cl:expt vz 2)
                              (cl:expt vw 2)))))
...
vectors VEC1 and VEC2 by FACTOR."
  (declare (optimize speed))
  (com:cwset 4 out (vec1 vec2) (cl:truncate (u:lerp factor vec1 vec2)))
  out)
...
component of vector VEC."
  (declare (optimize speed))
  (com:cwset 4 out vec (cl:floor vec divisor))
  out)
...
each component of vector VEC."
  (declare (optimize speed))
  (com:cwset 4 out vec (cl:ceiling vec divisor))
  out)
...
(defun mod! (out vec divisor)
  ...
  (declare (optimize speed))
  (com:cwset 4 out vec (com:smod32-mod vec divisor))
  out)
