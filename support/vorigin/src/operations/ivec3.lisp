...
(declaim (inline =))
(defun = (vec1 vec2)
  "Compare vectors VEC1 and VEC2 for equality. REL and ABS are the relative
and absolute tolerances to compare by, and should be tuned specially for
...
range of each component is bounded by MIN and MAX."
  ...
  (let ((diff (com:smod32- max min)))
    (com:cwset 3 out nil (com:smod32+ min (cl:random diff))))
  out)

(u:fn-> random (u:b32 u:b32) vec)
(declaim (inline random))
(defun random (min max)
  "Construct a fresh vector with random elements. The range of each
...
and VEC2."
  (declare (optimize speed))
  (com:cwset 3 out (vec1 vec2) (com:smod32+ vec1 vec2))
  out)
...
(declaim (inline /!))
(defun /! (out vec1 vec2)
  "Modify vector OUT by performing component-wise division of vectors VEC1
and VEC2."
  (declare (optimize speed))
  (com:cwset 3 out (vec1 vec2) (if (zerop vec2) 0 (com:smod32/ vec1 vec2)))
  out)

(u:fn-> / (vec vec) vec)
(declaim (inline /))
(defun / (vec1 vec2)
  "Construct a fresh vector by performing component-wise division of
vectors VEC1 and VEC2."
  (declare (optimize speed))
  (/! (zero) vec1 vec2))
...
(defun scale! (out vec scalar)
  "Modify vector OUT by adding the scalar SCALAR to each component of
vector VEC and truncating towards zero."
  (declare (optimize speed))
  (com:cwset 3 out vec (com:smod32* vec scalar))
  out)

(u:fn-> scale (vec u:b32) vec)
(declaim (inline scale))
(defun scale (vec scalar)
  "Construct a fresh vector by adding the scalar SCALAR to each component
of vector VEC."
  (declare (optimize speed))
...
(defun length-squared (vec)
  ...
  (with-components ((v vec))
    (com:smod32+ (cl:expt vx 2)
                 (com:smod32+ (cl:expt vy 2)
                              (cl:expt vz 2)))))
...
components of vector VEC to the nearest whole number. This is an
identity operation."
  (declare (optimize speed))
  (com:cwset 3 out vec vec)
  out)
...
(defun cross! (out vec1 vec2)
  "Modify vector OUT to be the result of the cross product between vectors
VEC1 and VEC2."
  (declare (optimize speed))
  (with-components ((o out) (v1 vec1) (v2 vec2))
    (psetf ox (com:smod32- (com:smod32* v1y v2z) (com:smod32* v1z v2y))
           oy (com:smod32- (com:smod32* v1z v2x) (com:smod32* v1x v2z))
           oz (com:smod32- (com:smod32* v1x v2y) (com:smod32* v1y v2x))))
  out)

(u:fn-> cross (vec vec) vec)
(declaim (inline cross))
(defun cross (vec1 vec2)
  "Construct a fresh vector with the result of the cross product between
vectors VEC1 and VEC2."
  (declare (optimize speed))
...
vectors VEC1 and VEC2 by FACTOR and truncating towards zero the result."
  (declare (optimize speed))
  (com:cwset 3 out (vec1 vec2) (cl:truncate (u:lerp factor vec1 vec2)))
  out)
...
between vectors VEC1 and VEC2 by FACTOR."
  (declare (optimize speed))
...
component of vector VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:floor vec divisor))
  out)
...
each component of vector VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:ceiling vec divisor))
  out)
...
(defun mod! (out vec divisor)
  ...
  (declare (optimize speed))
  (com:cwset 3 out vec (com:smod32-mod vec divisor))
  out)
