...
(define-function hermite ((point :vec2))
  (hermite point (lambda ((x :vec2)) (fl.gpu.hash:fast32/2-per-corner x))))
...
(define-function hermite/derivs ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (grad-x (* grad-x norm))
           (grad-y (* grad-y norm))
           (temp-x (fl.gpu.shaping:quintic-hermite
                    (.y vec) (.xy grad-x) (.zw grad-x) (.xy grad-y) (.zw grad-y)))
           (temp-y (fl.gpu.shaping:quintic-hermite
                    (.x vec) (.xz grad-y) (.yw grad-y) (.xz grad-x) (.yw grad-x)))
           (noise (fl.gpu.shaping:quintic-hermite
                   (.x vec) (.x temp-x) (.y temp-x) (.z temp-x) (.w temp-x)))
           (noise (map-domain noise -0.4419417 0.4419417 0 1))
           (derivs (* (vec2 (fl.gpu.shaping:quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x) (.w temp-x))
                            (fl.gpu.shaping:quintic-hermite/derivative
                             (.y vec) (.x temp-y) (.y temp-y) (.z temp-y) (.w temp-y)))
                      1.1313709)))
    (vec3 noise derivs)))

(define-function hermite/derivs ((point :vec2))
  (hermite/derivs point (lambda ((x :vec2)) (fl.gpu.hash:fast32/2-per-corner x))))
...
(define-function hermite ((point :vec3))
  (hermite point (lambda ((x :vec3)) (fl.gpu.hash:fast32/3-per-corner x))))
...
(define-function hermite/derivs ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival-z igrad-xz igrad-yz (fl.gpu.shaping:quintic-hermite
                                      (.z vec) grad-x0 grad-x1 grad-y0 grad-y1 grad-z0 grad-z1))
           (ival-y igrad-xy igrad-zy (fl.gpu.shaping:quintic-hermite
                                      (.y vec)
           ...
           (temp-z (fl.gpu.shaping:quintic-hermite
                    (.x vec)
           ...
           (derivs (* (vec3 (fl.gpu.shaping:quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x) (.w temp-x))
                            (fl.gpu.shaping:quintic-hermite/derivative
                             (.y vec) (.x temp-y) (.y temp-y) (.z temp-y) (.w temp-y))
                            (fl.gpu.shaping:quintic-hermite/derivative
...
(define-function hermite/derivs ((point :vec3))
  (hermite/derivs point (lambda ((x :vec3)) (fl.gpu.hash:fast32/3-per-corner x))))
