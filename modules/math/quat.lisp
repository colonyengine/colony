...
(fu:define-constant +zero-quat+
    (%quat 0f0 0f0 0f0 0f0)
  :test #'equalp)

(fu:define-constant +id-quat+
...
(defspecialization (quat :inline t) ((in mat3)) quat
  (with-quat ((q (quat 1)))
    (with-mat3 ((m in))
      (let ((tr1 (1+ (cl:- m.00 m.11 m.22)))
            (tr2 (cl:+ (cl:- 1 m.00) (cl:- m.11 m.22)))
            (tr3 (cl:+ (cl:- 1 m.00 m.11) m.22))
            (s 0f0))
        (cond
          ((and (cl:> tr1 tr2) (cl:> tr1 tr3))
           (setf s (cl:* (cl:sqrt tr1) 2)
                 q.w (cl:/ (cl:- m.21 m.12) s)
                 q.x (cl:* 0.25f0 s)
                 q.y (cl:/ (cl:+ m.01 m.10) s)
                 q.z (cl:/ (cl:+ m.02 m.20) s)))
          ((and (cl:> tr2 tr1) (cl:> tr2 tr3))
           (setf s (cl:* (cl:sqrt tr2) 2)
                 q.w (cl:/ (cl:- m.02 m.20) s)
                 q.x (cl:/ (cl:+ m.01 m.10) s)
                 q.y (cl:* 0.25f0 s)
                 q.z (cl:/ (cl:+ m.12 m.21) s)))
          (t
           (setf s (cl:* (cl:sqrt tr3) 2)
                 q.w (cl:/ (cl:- m.10 m.01) s)
                 q.x (cl:/ (cl:+ m.02 m.20) s)
                 q.y (cl:/ (cl:+ m.12 m.21) s)
                 q.z (cl:* 0.25f0 s))))))
    q))
  ...
  (with-quat ((q1 in1) (q2 in2) (o out))
    (let ((dot (dot q1 q2))
      ...
      (if (cl:> (cl:abs dot) 0.9995f0)
          (psetf o.w (fu:lerp factor q1.w q2.w)
                 o.x (fu:lerp factor q1.x q2.x)
                 o.y (fu:lerp factor q1.y q2.y)
                 o.z (fu:lerp factor q1.z q2.z))
          (let* ((angle (cl:acos (fu:clamp dot 0 1)))
