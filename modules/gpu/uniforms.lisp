...
(cl:defun store-uniforms (program)
...
(cl:defun store-uniform-locations (program)
  (let ((id (id program)))
    (gl:use-program id)
...
(cl:defun get-uniform-location (program-name uniform)
  (let ((program (find-program program-name)))
    (fl.util:href (uniforms program) uniform :location)))

(cl:defmacro %uniform-array (location func component-count element-type sequence)
...
(cl:defun uniform-int (program-name uniform value)
...
(cl:defun uniform-int-array (program-name uniform value)
...
(cl:defun uniform-float (program-name uniform value)
  "Specify a float as the VALUE for the uniform variable, UNIFORM."
  (let ((location (get-uniform-location program-name uniform)))
    (%gl:uniform-1f location value)))

(cl:defun uniform-float-array (program-name uniform value)
...
(cl:defun uniform-vec3 (program-name uniform value)
...
(cl:defun uniform-vec3-array (program-name uniform value)
...
(cl:defun uniform-vec4 (program-name uniform value)
...
(cl:defun uniform-vec4-array (program-name uniform value)
