...
(define-function perlin ((point :vec2))
  (perlin point (lambda ((x :vec2)) (fl.gpu.hash:fast32/2-per-corner x))))
...
(define-function perlin-improved ((point :vec2))
  (perlin-improved point (lambda ((x :vec2)) (fl.gpu.hash:fast32 x))))
...
(define-function perlin ((point :vec3))
  (perlin point (lambda ((x :vec3)) (fl.gpu.hash:fast32/3-per-corner x))))
...
(define-function perlin-surflet ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (out (* (+ (dot (fl.gpu.shaping:falloff-squared-c2
                           ...
                           temp1)
                      (dot (fl.gpu.shaping:falloff-squared-c2
...
(define-function perlin-improved ((point :vec3))
  (perlin-improved point (lambda ((x :vec3)) (fl.gpu.hash:fast32 x))))
...
(define-function perlin ((point :vec4))
  (perlin point (lambda ((x :vec4)) (fl.gpu.hash:fast32-2/4-per-corner x))))
