...
(defun cubist ((point :vec2)
               (range-clamp :vec2))
  (cubist point range-clamp (lambda ((x :vec2)) (fast32/3-per-corner x))))
...
(defun cubist ((point :vec3)
               (range-clamp :vec2))
  (cubist point range-clamp (lambda ((x :vec3)) (fast32/4-per-corner x))))
...
(defun stars ((point :vec2)
  ...
  (let* ((cell (floor point))
    ...
    (if (< (.w hash) probability-threshold)
        (* (falloff-squared-c1 (min (dot vec vec) 1)) value)
        0.0)))
