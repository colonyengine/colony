...
(defun value ((point :vec2)
  ...
  (let* ((cell (floor point))
         ...
         (blend (quintic-curve vec))
         (blend (vec4 blend (- 1 blend))))
    (dot hash (* (.zxzx blend) (.wwyy blend)))))

(defun value ((point :vec2))
  (value point (lambda ((x :vec2)) (fast32 x))))
...
(defun value/derivs ((point :vec2)
  ...
  (let* ((cell (floor point))
         (vec (- point cell))
         (hash (funcall hash-fn cell))
         (blend (quintic-curve/interpolate-derivative vec))
         (out (mix (.xyxz hash) (.zwyw hash) (.yyxx blend))))
    (+ (vec3 (.x out) 0 0)
       (* (- (.yyw out) (.xxz out)) (.xzw blend)))))

(defun value/derivs ((point :vec2))
  (value/derivs point (lambda ((x :vec2)) (fast32 x))))
...
(defun value ((point :vec3))
  (value point (lambda ((x :vec3)) (fast32 x))))
  ...
  (mvlet* ((cell (floor point))
    ...
    (+ (vec4 (.x temp1) 0 0 0)
       (* (- (vec4 (.yyw temp1) (.y temp2)) (vec4 (.xxz temp1) (.x temp2)))
          (vec4 (.x blend) (quintic-curve/derivative vec))))))

(defun value/derivs ((point :vec3))
  (value/derivs point (lambda ((x :vec3)) (fast32 x))))
...
(defun value ((point :vec4))
  (value point (lambda ((x :vec4)) (fast32-2 x))))
...
(defun value-hermite ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (out (quintic-hermite
                 ...
                 (vec4 (.xy hash-z) 0 0)
                 (vec4 (.zw hash-z) 0 0)))
           (out (* (quintic-hermite (.x vec) (.x out) (.y out) (.z out) (.w out))
                   normalization-value)))
...
(defun value-hermite ((point :vec2)
  ...
  (value-hermite point value-scale gradient-scale normalization-value
                 (lambda ((x :vec2)) (fast32/3-per-corner x))))
...
(defun value-hermite ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival igrad-x igrad-y (quintic-hermite
...
(defun value-hermite ((point :vec3)
                      ...
                      (normalization-value :float))
  (value-hermite point value-scale gradient-scale normalization-value
                 (lambda ((x :vec3)) (fast32/4-per-corner x))))
...
(defun value-perlin ((point :vec2)
                     (blend-value :float))
  (value-perlin point blend-value (lambda ((x :vec2)) (fast32/3-per-corner x))))
...
(defun value-perlin ((point :vec3)
                     (blend-value :float))
  (value-perlin point blend-value (lambda ((x :vec3)) (fast32/4-per-corner x))))
