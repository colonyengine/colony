...
(defun polkadot ((point :vec2)
  ...
  (let* ((cell (floor point))
         ...
         (vec (+ (* (.xy hash) (- radius 2))
                 (- (* vec radius) (1- radius)))))
    (* (falloff-squared-c2 (min (dot vec vec) 1.0)) value)))

(defun polkadot ((point :vec2)
                 (radius-low :float)
                 (radius-high :float))
  (polkadot point radius-low radius-high (lambda ((x :vec2)) (fast32/cell x))))
...
(defun polkadot-box ((point :vec2)
                     (radius-low :float)
                     (radius-high :float))
  (polkadot-box point radius-low radius-high (lambda ((x :vec2)) (fast32/cell x))))
...
(defun polkadot ((point :vec3)
  ...
  (let* ((cell (floor point))
         ...
         (vec (+ (- (* vec radius) (1- radius))
                 (* (.xyz hash) (- radius 2)))))
    (* (falloff-squared-c2 (min (dot vec vec) 1)) value)))

(defun polkadot ((point :vec3)
                 (radius-low :float)
                 (radius-high :float))
  (polkadot point radius-low radius-high (lambda ((x :vec3)) (fast32/cell x))))
...
(defun polkadot-box ((point :vec3)
                     (radius-low :float)
                     (radius-high :float))
  (polkadot-box point radius-low radius-high (lambda ((x :vec3)) (fast32/cell x))))
