...
(defun hermite ((point :vec2))
  (hermite point (lambda ((x :vec2)) (fast32/2-per-corner x))))
...
(defun hermite/derivs ((point :vec2))
  (hermite/derivs point (lambda ((x :vec2)) (fast32/2-per-corner x))))
...
(defun hermite ((point :vec3))
  (hermite point (lambda ((x :vec3)) (fast32/3-per-corner x))))
...
(defun hermite/derivs ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival-z igrad-xz igrad-yz (quintic-hermite
                                      (.z vec) grad-x0 grad-x1 grad-y0 grad-y1 grad-z0 grad-z1))
           (ival-y igrad-xy igrad-zy (quintic-hermite
                                      (.y vec)
           ...
           (derivs (* (vec3 (quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x) (.w temp-x))
                            (quintic-hermite/derivative
                             (.y vec) (.x temp-y) (.y temp-y) (.z temp-y) (.w temp-y))
                            (quintic-hermite/derivative
...
(defun hermite/derivs ((point :vec3))
  (hermite/derivs point (lambda ((x :vec3)) (fast32/3-per-corner x))))
