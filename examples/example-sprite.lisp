...
(defmethod v:on-component-update ((self shot-emitter))
  (let ((context (v:context self)))
    (when (or (v:input-enter-p context '(:gamepad1 :a))
      ...
      (let* ((parent-model (comp:model (transform self)))
  ...
  ("ship"
   ...
   ("ship-body"
    (comp:sprite :spec :spritesheet-data
                 :name "ship29")
    (comp:render :material `(contrib.mat:sprite
                             ,(a:make-gensym '#:sprite)
                             :uniforms ((:sprite.sampler sprites)))
                 :mode :sprite)
    ("exhaust"
     (comp:transform :translate (v3:vec 0 -140 0))
     (comp:sprite :spec :spritesheet-data
                  :name "exhaust03-01"
                  :frames 8)
     (comp:render :material `(contrib.mat:sprite
...
(v:define-prefab "sprite-2" (:library examples)
  ...
  ("plane"
   ...
   (comp:render :material `(contrib.mat:sprite
                               ...
                               :uniforms ((:sprite.sampler sprites)))
                   :mode :sprite)
   (comp:actions :default-actions '((:type contrib.action:rotate
                                        :duration 4
