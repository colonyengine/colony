...
(fl:define-scene isometric-view-test ()
  ...
  (@isocam-45
   ((fl.comp:transform :rotate (m:vec3 0 0 (- (/ pi 4.0)))))
   (@isocam-35.264
    ((fl.comp:transform :rotate (m:vec3 (- (atan (/ (sqrt 2.0)))) 0 0)))
    (@camera
     ;; translate down y axis, rotate to look at origin again.
     ((fl.comp:transform :translate (m:vec3 0 -5 0)
                         :rotate (m:vec3 (+ (/ pi 2.0)) 0 0))
      (fl.comp:camera :active-p t
                      :mode :orthographic
                      :zoom 100)))))
  (@plane1
   ((fl.comp:transform :translate (m:vec3)
