...
(defun input-transition-in (core-state input)
  (symbol-macrolet ((state (fl.util:href (states (input-data core-state)) input)))
...
(defun input-transition-out (core-state input)
  (fl.util:when-let ((state (fl.util:href (states (input-data core-state)) input)))
    (with-slots (enter enabled exit) state
...
(defun enable-entering (core-state)
  (symbol-macrolet ((entering (entering (input-data core-state))))
    (dolist (input entering)
      (with-slots (enter enabled exit) (fl.util:href (states (input-data core-state)) input)
...
(defun input-enter-p (context input)
  (fl.util:when-let ((state (fl.util:href (states (input-data (core-state context))) input)))
