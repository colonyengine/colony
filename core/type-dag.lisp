  ...
  (loop :for gdef :in (fl.util:hash-values (graphdefs graph))
        ...
        :do (setf (depends-on gdef) (fl.util:dict #'eq))
            (loop :for (gdef-name subform-names) :in whole-depends-on-form
                  :for gdef-reference = (fl.util:href (graphdefs graph) gdef-name)
...
(defun lookup-splice (splice-form gdef)
  ...
  (let ((splice-name (second splice-form)))
    ...
    (dolist (dep-inst (fl.util:hash-values (depends-on gdef)))
      (fl.util:when-found (subform (fl.util:href (subforms dep-inst) splice-name))
...
(defmethod generate-graph-annotation ((category (eql 'component-dependency)) graph)
  (let* ((clg (graph graph))
         ...
         (annotation (make-graph-annotation
                      (category graph)
                      :unknown-type-id (fl.util:unique-name "UNKNOWN-TYPE-ID-"))))
     ...
     (lambda (v)
       (let ((elem-v (cl-graph:element v)))
         (when (is-syntax-form-p '(component-type) elem-v)
           (setf (fl.util:href (referenced-types annotation) (second elem-v)) t)))))
     ...
     (lambda (v)
       (let* ((elem-v (cl-graph:element v))
         ...
         (dolist (pkg-name all-packages)
           (fl.util:when-let* ((matched-pkg-name (ppcre:scan-to-strings
                                                  putative-package-name-regex pkg-name))
                               (found-pkg (find-package matched-pkg-name)))
             (pushnew found-pkg (fl.util:href (pattern-matched-packages annotation) second)))))))
    (setf (annotation graph) annotation)))
...
return the unknown-type-id symbol."
  (let* ((putative-component-type component-type)
    ...
    (assert annotation)
    (unless (fl.util:href (referenced-types annotation) putative-component-type)
    ...
    `(symbol-macrolet ((,graphs (fl.data:get 'graphs))
       ...
       (let ((,graph (parse-graph-definition ',name ',category ',depends-on ',roots ',body)))
         ...
         (setf (fl.util:href ,graphs ',category) (make-analyzed-graph :category ',category)
               (fl.util:href (graphdefs ,analyzed-graph) ',name) ,graph)))))
