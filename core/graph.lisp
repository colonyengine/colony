...
pairs, :hyperedges"
  (let* ((canonical-form (canonicalize-dependency-form category form))
         (x (u:split-sequence '-> canonical-form :test #'eql/package-relaxed))
        ...
        :do (setf (depends-on gdef) (u:dict #'eq))
            (loop :for (gdef-name subform-names) :in whole-depends-on-form
                  ...
                  :do (assert gdef-reference)
                      ...
                      (if (eq subform-names :all)
                          ...
                          (maphash
                           (lambda (subform-name subform-instance)
                             (setf (u:href (subforms analyzed-depends-on) subform-name)
                                   subform-instance))
                          ...
                          (dolist (subform-name subform-names)
                            (setf (u:href (subforms analyzed-depends-on) subform-name)
                                  (u:href (subforms gdef-reference) subform-name))))
                      ...
                      (setf (u:href (depends-on gdef) (name analyzed-depends-on))
                            analyzed-depends-on))))
...
(defmethod generate-graph-annotation ((category (eql 'component-dependency)) graph)
  (let* ((clg (graph graph))
         ...
         (annotation (make-graph-annotation
                      (category graph)
                      :unknown-type-id (u:unique-name "UNKNOWN-TYPE-ID-"))))
       ...
       (let* ((elem-v (cl-graph:element v))
         ...
         (dolist (pkg-name all-packages)
           (u:when-let* ((matched-pkg-name (ppcre:scan-to-strings
             ...
             (pushnew found-pkg (u:href (pattern-matched-packages annotation) second)))))))
    (setf (annotation graph) annotation)))
...
return it, otherwise return the unknown-type-id symbol."
  (let* ((putative-component-type component-type)
         (component-dependency-graph (u:href (analyzed-graphs core-state) 'component-dependency))
...
(defmacro define-graph (&whole form name (&key category depends-on roots) &body body)
  ...
  (u:with-unique-names (definitions)
    `(symbol-macrolet ((,definitions (fl.data:get 'graphs)))
       (unless ,definitions
         (fl.data:set 'graphs (u:dict #'eq)))
...
(defun load-graphs (core-state)
  (with-slots (%analyzed-graphs) core-state
    ...
    (u:do-hash-values (graph-code (fl.data:get 'graphs))
