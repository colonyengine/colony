...
(define-texture-profile default-profile
  ...
  ;; NOTE: This next one might be called :depth-texture-mode in cl-opengl.
  ;; Or, :dempth-texture-mode might have been removed and replaced with this.
  ...
  ;; The next one below is :nearest-mipmap-linear default in ogl. But,
  ;; this one is almost surely what you actually want. Also, it will
  ;; downconvert to :nearest or :linear as apprpriate if you turn of mipmaps
  ;; for a texture.
  ...
  ;; NOTE: The images defined in the texture are exactly those that will be
  ;; put into the texture. Meaning, if base-level is 0, and max-level is 1000,
  ;; and there are 11 images defined, then the resolution of image 0 in the
  ;; ordered array must be such that 11 images are created via the mipmap
  ;; process. If base_level is 4 and max-level is 10, then there should be 6
  ;; mipmaps following the rules of halving:
