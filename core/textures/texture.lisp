...
(defun add-semantic-texture-descriptor (texdesc core-state)
  (setf (au:href (semantic-texture-descriptors (textures core-state)) (name texdesc)) texdesc))
...
(defun get-procedural-texture-descriptors (context)
  ...
  (let ((results))
    (au:do-hash-values (texdesc (semantic-texture-descriptors (textures (core-state context))))
...
(defun find-semantic-texture-descriptor (semantic-texture-name context)
  (au:href (semantic-texture-descriptors (textures (core-state context))) semantic-texture-name))
...
(defun (setf get-semantic-applied-attribute) (newval texture attribute-name)
  (setf (au:href (applied-attributes (semantic-texdesc texture)) attribute-name) newval))
...
(defun (setf get-computed-applied-attribute) (newval texture attribute-name)
  (setf (au:href (applied-attributes (computed-texdesc texture)) attribute-name) newval))
  ...
  (with-accessors ((texture-type texture-type) (applied-attributes applied-attributes))
    ...
    (let ((texture-parameters
      ...
      (loop :for putative-parameter :in texture-parameters
            :do (au:when-found (value (au:href applied-attributes putative-parameter))
  ...
  (au:with-unique-names (profile)
    (let ((definition '(fl.data:get 'texture-profiles)))
      `(let ((,profile ,(parse-texture-profile name body)))
         (unless ,definition
           (fl.data:set 'texture-profiles (au:dict #'eq)))
    ...
    (let ((definition '(fl.data:get 'textures)))
      `(let ((,desc (make-texture-descriptor
         ...
         (setf ,@(loop :for (key value) :in body
                       :append `((au:href (attributes ,desc) ,key) ,value)))
         (unless ,definition
           (fl.data:set 'textures (au:dict #'eq)))
