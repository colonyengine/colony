...
The resource cache stores (usually) deduplicated data once in the engine and
...
when derivations don't have to happen and the reslting code needs to choose
...
and the code itself is authoritative:
  - `cache-item` API:
    - **make-cache-item**, Function: 
      Create a new base `cache-item` insstance.
  - `cache-domain` API:
