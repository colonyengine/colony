...
(fu:define-printer (actor stream :type t)
...
(defun attach-component (actor component)
  ...
  (setf (fu:href (components actor) component) component)
  (let ((qualified-type (qualify-component (core-state actor) (component-type component))))
    (push component (fu:href (components-by-type actor) qualified-type))))
...
(defun detach-component (actor component)
  ...
  (when (remhash component (components actor))
    (symbol-macrolet ((typed-components (fu:href (components-by-type actor)
...
(defun actor-components-by-type (actor component-type)
  ...
  (let ((qualified-type
    ...
    (fu:href (components-by-type actor) qualified-type)))
...
actor."
  (let ((core-state (core-state context))
    ...
    (setf (fu:href (actor-preinit-db (tables core-state)) actor) actor)
    (fu:do-hash-values (v (components actor))
