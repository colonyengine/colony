...
(defmethod action:on-update (action (type (eql 'rotate)))
  (let* ((transform (c/render::transform
    ...
    (ecase (or (u:href attrs :axis) :z)
      (:x (v:rotate transform (q:orient :local :x step) :replace t))
      (:y (v:rotate transform (q:orient :local :y step) :replace t))
      (:z (v:rotate transform (q:orient :local :z step) :replace t)))))
