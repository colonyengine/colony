...
(defmethod load-texture-data ((texture-type (eql :texture-2d)) texture context)
  ...
  (let* ((use-mipmaps-p (get-computed-applied-attribute texture :use-mipmaps))
    ...
    (let ((images (read-texture-mipmaps texture :2d context)))
...
(defmethod read-texture-mipmaps (texture (kind (eql :2d)) context)
  (let* ((use-mipmaps-p (get-computed-applied-attribute texture :use-mipmaps))
    ...
    (let ((images (read-mipmap-images context data use-mipmaps-p :2d flip-y)))
      ...
      (loop :with max-size = c::=max-texture-size=
            ...
            :do (when (> (max (img:height image) (img:width image))
                         max-size)
                  (error
                   "Image ~a for 2D texture ~a is too big to be loaded onto ~
...
(defmethod gpu-allocate-texture-storage ((texture-type (eql :texture-2d))
                                         texture images)
...
(defmethod gpu-upload-texture-mipmap-layer ((texture-type (eql :texture-2d))
                                            texture image level
