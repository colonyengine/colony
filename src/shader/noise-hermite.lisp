...
(define-function hermite ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival igrad-x igrad-y (shaping:quintic-hermite
...
(define-function hermite/derivs ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival-z igrad-xz igrad-yz (shaping:quintic-hermite
                                      (.z vec) grad-x0 grad-x1 grad-y0 grad-y1
                                      grad-z0 grad-z1))
           (ival-y igrad-xy igrad-zy (shaping:quintic-hermite
                                      (.y vec)
           ...
           (temp-z (shaping:quintic-hermite
                    (.x vec)
           ...
           (derivs (* (vec3 (shaping:quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x)
                             (.w temp-x))
                            (shaping:quintic-hermite/derivative
                             (.y vec) (.x temp-y) (.y temp-y) (.z temp-y)
