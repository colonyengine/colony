  ...
  (let ((actors (u:dict #'equalp))
    ...
    (u:do-hash (path node (parse-tree prefab))
      (with-slots (%name %id %display-id) node
        (let ((actor (c:make-actor context
                                   :prefab-node node
                                   :id %id
...
(defun make-actor-components (context actors)
  (let ((components (u:dict #'eq)))
    (u:do-hash-values (actor actors)
      (u:do-hash (type table (components-table (c::prefab-node actor)))
        ...
        (u:do-hash (id data table)
          ...
          (let ((component (c:make-component context type)))
            ...
            (c:attach-component actor component)))))
...
(defun make-actor-relationships (context prefab actors parent)
  (let ((parent (or parent (c::scene-tree (c::core context))))
        (root (u:href actors (path (root prefab)))))
    (u:do-hash-values (actor actors)
      (let ((node (c::prefab-node actor)))
        (u:do-hash-values (child (children node))
          (comp:add-child
           (c:component-by-type actor 'comp:transform)
           (c:component-by-type (u:href actors (path child))
                                'comp:transform)))))
    (comp:add-child
     (c:component-by-type parent 'comp:transform)
     (c:component-by-type root 'comp:transform))))

(defun make-factory (prefab)
  (lambda (core &key parent)
    (u:mvlet* ((context (c:context core))
      ...
      (u:do-hash-values (actor actors)
        (c:spawn-actor actor))
      (push root (u:href (c::prefab-entities core) spec))
      root)))
